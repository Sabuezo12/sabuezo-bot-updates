...
cavebotMacro = macro(20, function()
  ...
  if TargetBot and TargetBot.isActive() and not TargetBot.isCaveBotActionAllowed() and not canRunWhileTargeting(currentAction) then
  ...
  if CaveBot._profileChangedByAction then
    CaveBot._profileChangedByAction = false
    actionRetries = 0
