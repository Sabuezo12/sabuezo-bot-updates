...
local function registerSupplyCounter(id, data)
  id = tonumber(id)
  if not id or id <= 0 then return end
  if vBot.ItemCounter and vBot.ItemCounter.registerSupplyItem then
    vBot.ItemCounter.registerSupplyItem(id, data)
...
Supplies.getItemsData = function()
  local t = {}
  if vBot.ItemCounter and vBot.ItemCounter.clearSupplyItems then
    vBot.ItemCounter.clearSupplyItems()
  end
  -- items
  for i, panel in ipairs(SuppliesWindow.items:getChildren()) do
    if panel.id:getItemId() > 100 then
